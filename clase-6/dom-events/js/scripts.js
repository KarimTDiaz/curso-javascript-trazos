...
- En un HTML vacío crea un h1 y un h2 con el texto "Soy un título" y "Soy un subtítulo".
  ...
  document.body.addEventListener('click', event => {
    if(event.target.tagName === 'BUTTON'){
        event.target.previousElementSibling.textContent = 'Soy un Subtítitulo';
        event.target.previousElementSibling.previousElementSibling.textContent = 'Soy un Título';
    } else {
        event.target.textContent += ' He sido clickado'
    }
  });
